...
router.post('/register',async(req,res)=>{
  ...
  if(JSON.stringify(errMsg) !== '{}'){
    ...
    try{
      ...
      if(!finData){
        let result = await usersModel.create({email,pwd:sha1(pwd),name})
        console.log(result);
        //res.send('注册成功！')
        //res.render('login')
...
router.post('/login',async(req,res)=>{
  ...
  if(JSON.stringify(errMsg) !== '{}'){
    ...
    try{
      let resData = await usersModel.findOne({email,pwd:sha1(pwd)})
      if(!resData){
        //res.send('登录失败，用户名或密码错误')
        let errMsg = {loginErr:'登录失败，用户名或密码错误'}
        res.render('login',{errMsg})
      }else{
        //向客户端“种”一个cookie
        //res.cookie('name',resData._id)
        req.session.userid = resData._id
        //redirect属于客户端跳转，让客户端再次发出请求
